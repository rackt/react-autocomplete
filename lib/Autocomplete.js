...
let Autocomplete = React.createClass({
  ...
  getDefaultProps () {
    return {
      ...
      onChange () {},
      onSelect (value, item) {},
      renderMenu (items, value, style, component) {
        if (component.props.items.length > component.props.maxItem) {
          items.push(<div>Keep typing to refine results</div>)
        }
  ...
  getFilteredItems () {
    ...
    if (items.length > this.props.maxItem) {
      items = items.slice(0, this.props.maxItem)
    }
  ...
  renderMenu () {
    ...
    var menu = this.props.renderMenu(items, this.state.value, style, this)
