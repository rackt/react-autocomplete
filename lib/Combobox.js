var ComboboxOption = require('./ComboboxOption');
...
var Combobox = React.createClass({
  ...
  componentWillMount: function() {
    this.setState({menu: this.getMenuFromChildren(this.props.children)});
  },

  componentWillReceiveProps: function(newProps) {
    this.setState({menu: this.getMenuFromChildren(newProps.children)});
  },
  ...
  */
  getMenuFromChildren: function(children) {
    var activeChildId;
    var isEmpty = true;

    var wrappedChildren = React.Children.map(children, (child, index) => {
      if (!child instanceof ComboboxOption) {
        // Allow children that aren't ComboboxOption to live in the menu.
        // Useful for empty menus (i.e. when you want to display a "no items" 
        // message) and menus that contain other controls in the popup.
        return;
      }
      ...
      if (this.state.value === child.props.value) {
        ...
        additionalProps.id = child.props.id || 'rf-combobox-selected-'+(++guid);
        additionalProps.isSelected = true;
        activeChildId = additionalProps.id;
      }

      additionalProps.onBlur = this.handleOptionBlur;
      additionalProps.onClick = this.selectOption.bind(this, child);
      additionalProps.onFocus = this.handleOptionFocus;
      additionalProps.onKeyDown = this.handleOptionKeyDown.bind(this, child);
      additionalProps.onMouseEnter = 
        this.handleOptionMouseEnter.bind(this, index);
      
      return cloneWithProps(child, additionalProps);
    });
  ...
  getClassName: function() {
    var className = joinClasses(this.props.className, 'rf-combobox');
    if (this.state.isOpen)
      className = joinClasses(className, 'rf-combobox--is-open');
    return className;
  },
    ...
    return (
      <div className={this.getClassName()}>
        <input
          ...
          role="combobox"
          aria-activedescendant={this.state.menu.activeChildId}
