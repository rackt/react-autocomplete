...
describe('Autocomplete isClearable', () => {

  const autocompleteWrapper = mount(AutocompleteComponentJSX({
    value: 'Test',
    isClearable: false,
    onClear: () => { autocompleteWrapper.setProps({ value: '' }) },
  }))
  let autocompleteClearButton
  it('should render a clear button' , () => {
    autocompleteWrapper.setProps({ isClearable: true })
    autocompleteClearButton = autocompleteWrapper.find('.fa-times-circle')
    expect(autocompleteClearButton.exists()).toBe(true)
  })

  it('should invoke a function which clears the value when clear button is clicked', () => {
    autocompleteClearButton.simulate('click')
    expect(autocompleteWrapper.prop('value')).toBe('')
  })
})
