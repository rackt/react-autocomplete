...
describe('Autocomplete acceptance tests', () => {
  ...
  it('should display autocomplete menu when input has focus', () => {
    ...
    expect(autocompleteWrapper.instance().refs.menu).not.toBe(false);
    expect(autocompleteWrapper.instance().refs.menu.style.display).toBe('none');
    ...
    expect(autocompleteWrapper.instance().refs.menu.style.display).toBe('block');
  });
  ...
  it('should display menu based on `props.open` when provided', () => {
    ...
    expect(tree.find('> div').getDOMNode().style.display).toBe('none');
    tree.setState({ isOpen: true });
    expect(tree.find('> div').length).toBe(1);
    expect(tree.find('> div').getDOMNode().style.display).toBe('block');
